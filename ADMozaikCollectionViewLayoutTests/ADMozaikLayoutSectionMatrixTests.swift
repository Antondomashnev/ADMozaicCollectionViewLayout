...
class ADMozaikLayoutSectionMatrixFillModeTests: XCTestCase {
    
    var matrixLayout: ADMozaikLayoutSectionMatrix!
    
    override func setUp() {
        super.setUp()
        self.matrixLayout = ADMozaikLayoutSectionMatrix(numberOfColumns: 3, section: 1, contentMode: .fill)
    }
...
class ADMozaikLayoutSectionMatrixOrderedModeTests: XCTestCase {
    
    var matrixLayout: ADMozaikLayoutSectionMatrix!
    
    override func setUp() {
        super.setUp()
        self.matrixLayout = ADMozaikLayoutSectionMatrix(numberOfColumns: 2, section: 1, contentMode: .ordered)
    }
    ...
    func testThatFilledMatrixShouldExtendMatrixHeightIfThereIsNoSpaceLeft() {
        do {
            ...
            try self.matrixLayout.addItem(of: ADMozaikLayoutSize(numberOfColumns: 1, numberOfRows: 1), at: position)
            position = try self.matrixLayout.positionForItem(of: ADMozaikLayoutSize(numberOfColumns: 2, numberOfRows: 1))
            try self.matrixLayout.addItem(of: ADMozaikLayoutSize(numberOfColumns: 2, numberOfRows: 1), at: position)
            expect(position.column).to(equal(0))
            expect(position.row).to(equal(2))
            expect(position.section).to(equal(1))
        }
        catch { fatalError() }
    }
    ...
    func testThatPositionForItemWithSizeShouldReturnCorrectPosition() {
        do {
            ...
            position = try self.matrixLayout.positionForItem(of: ADMozaikLayoutSize(numberOfColumns: 2, numberOfRows: 2))
            try self.matrixLayout.addItem(of: ADMozaikLayoutSize(numberOfColumns: 2, numberOfRows: 2), at: position)
            expect(position.column).to(equal(0))
            expect(position.row).to(equal(3))
            expect(position.section).to(equal(1))
            ...
            try self.matrixLayout.addItem(of: ADMozaikLayoutSize(numberOfColumns: 1, numberOfRows: 1), at: position)
            expect(position.column).to(equal(0))
            expect(position.row).to(equal(5))
            expect(position.section).to(equal(1))
        }
        catch { fatalError() }
    }
